...
class ChatViewController: UIViewController, UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    func handleMPCReceivedDataWithNotification(_ notification: Notification) {
        ...
        if let order = dataDictionary["order"] {
            if let user = dataDictionary["user"]{
                let orderFood: NSEntityDescription? = NSEntityDescription.entity(forEntityName: "Bill", in: context)
                if orderFood != nil {
                    let order1: Bill = Bill(entity: orderFood!, insertInto: context)
                    order1.order = order
                    order1.byUser = user
